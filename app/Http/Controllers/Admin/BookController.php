...
class BookController extends Controller
{
    ...
    public function get_data(Request $request)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $item) {
                $item->created_at_edited = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $item->created_at)->format('j F Y - H:i:s');
    ...
    public function do_create(Request $request)
    {
        ...
        $names = [
            ...
            'price' => ucwords(lang('price', $this->translation))
        ];
        $this->validate($request, $validation, $message, $names);

        // HELPER VALIDATION FOR PREVENT SQL INJECTION & XSS ATTACK
        // PROCESSING IMAGE
        $dir_path = 'uploads/book/';
        $image_file = $request->file('image');
        $format_image_name = time() . '-book';
        ...
        // SET ORDER / ORDINAL
        $last = Book::select('ordinal')->orderBy('ordinal', 'desc')->first();
        $ordinal = 1;
        if ($last) {
    ...
    public function edit($id)
    {
        ...
        return view('admin.book.form', compact('data'));
    }
